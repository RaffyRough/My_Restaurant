...
class Window(Client):
    ...
    def __init__(self):
        ...
        self.root = tk.Tk()
        self.display = tk.Text(self.root, height=10, width=45, bg='Orange', fg='White', bd=4)
        self.display.pack()
        self.root.geometry('500x500')
        self.root.title('Waiter Window')
        self.root.wm_resizable(width=False, height=False)
        self.price_display = tk.Text(self.root, height=3, width=15, bg='Orange', fg='White', bd=4)
        ...
        self.clear_button = ttk.Button(self.root, text='Clear Display')
        self.clear_button.place(x=210, y=360)
        self.clear_button.bind('<Button>', self.clearDisplay)
        self.clear_total = ttk.Button(self.root, text='Clear Total')
        self.clear_total.place(x=210, y=400)
        self.clear_total.bind('<Button>', self.clearTotal)

    #====================== Window Class Methods ==========================
    def run(self):
        self.root.mainloop()

    def clearDisplay(self, e):
    ...
    def netTotal(self, e):
        """:arg e -> event that is bound to execution of this func"""
        net_total = round(Window.total * 1.10, 2)
        self.price_display.delete('1.0', 'end')
        self.price_display.insert(tk.END, net_total)

    def clearTotal(self, e):
        """:arg e -> event that is bound to execution of this func"""
        self.price_display.delete('1.0', 'end')
        Window.total = 0
    ...
    def _write(self, order):
        """:arg order -> An order procured from onClick_1 func to be sent to the server
            This is a private function that is called by the onClick method and sends the 'order' to the server"""
